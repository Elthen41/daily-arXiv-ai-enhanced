...
function showPaperDetails(paper, paperIndex) {
  ...
  const promptText = `请你阅读这篇文章${paper.url.replace('abs', 'pdf')},总结一下这篇文章解决的问题、相关工作、研究方法、做了什么实验及其结果、结论，最后整体总结一下这篇文章的内容`;
  const chatgptLink = document.getElementById('chatgptLink');
  if (chatgptLink) {
    const encodedPrompt = encodeURIComponent(promptText);
    const chatgptBaseUrl = 'https://chat.openai.com/?model=gpt-5&input=';
    chatgptLink.href = `${chatgptBaseUrl}${encodedPrompt}`;
  }
